...
public class Tile {
    ...
    public void calculateH(int sx, int sy, int sz, int ex, int ey, int ez, boolean update) {
        // only handlers if h hasn't been calculated or if forced
    ...
    // onUpdate the grid, following the path generated to get there.
    public void calculateG(int sx, int sy, int sz, boolean update) {

        if ((!update && g == -1) || update) {

            // only handlers if g hasn't been calculated or if forced
